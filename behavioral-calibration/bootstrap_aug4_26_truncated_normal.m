...
%  Reporting errors follow a normal distribution truncated to [0,1].
%
%  Types:
...
%   - fraction of selected fits with sigma near its upper bound
...
opts.boundTol    = 1e-3;  % near-bound if within 0.1% of parameter range
opts.verbose     = false;
...
% Fraction of selected fits with sigma near its upper bound
shareSigmaUB_RI  = nan(B,3);
shareSigmaUB_RIR = nan(B,3);
shareSigmaUB_ER  = nan(B,3);
shareSigmaUB_EI  = nan(B,3);
...
%% 13. Save
save('bootstrap_results_subject_types_four_models_BIC_truncated_normal_bounded_sigma.mat', ...
    'shareRI','shareRIR','shareER','shareEI', ...
    'meanSigma_RI','meanSigma_RIR','meanSigma_ER','meanSigma_EI', ...
    'meanLambda_RI','meanLambda_RIR', ...
    'shareSigmaUB_RI','shareSigmaUB_RIR','shareSigmaUB_ER','shareSigmaUB_EI', ...
    'nRI','nRIR','nER','nEI', ...
...
fprintf('\nSaved bootstrap_results_subject_types_four_models_BIC_truncated_normal_bounded_sigma.mat\n');
...
fprintf('\n--- Fraction of selected fits with sigma near upper bound (%.4f) ---\n', opts.sigmaUB);
print_bootstrap_summary('Reaction-intensity sigma at upper bound - Control',   shareSigmaUB_RI(:,1));
print_bootstrap_summary('Reaction-intensity sigma at upper bound - Narrative', shareSigmaUB_RI(:,2));
print_bootstrap_summary('Reaction-intensity sigma at upper bound - Followup',  shareSigmaUB_RI(:,3));
print_bootstrap_summary('Reaction-intensity + reset sigma at upper bound - Control',   shareSigmaUB_RIR(:,1));
print_bootstrap_summary('Reaction-intensity + reset sigma at upper bound - Narrative', shareSigmaUB_RIR(:,2));
print_bootstrap_summary('Reaction-intensity + reset sigma at upper bound - Followup',  shareSigmaUB_RIR(:,3));
print_bootstrap_summary('Extreme reset sigma at upper bound - Control',   shareSigmaUB_ER(:,1));
print_bootstrap_summary('Extreme reset sigma at upper bound - Narrative', shareSigmaUB_ER(:,2));
print_bootstrap_summary('Extreme reset sigma at upper bound - Followup',  shareSigmaUB_ER(:,3));
print_bootstrap_summary('Extreme inertia sigma at upper bound - Control',   shareSigmaUB_EI(:,1));
print_bootstrap_summary('Extreme inertia sigma at upper bound - Narrative', shareSigmaUB_EI(:,2));
print_bootstrap_summary('Extreme inertia sigma at upper bound - Followup',  shareSigmaUB_EI(:,3));
...
function fit = fit_type1_reaction_intensity(y, s, b0, opts)
    obj = @(theta) nll_type1( ...
        bounded_transform(theta(1), opts.sigmaLB, opts.sigmaUB), ...
        bounded_transform(theta(2), opts.lambdaLB, opts.lambdaUB), ...
        y, s, b0, opts);
    theta0 = [inverse_bounded_transform(0.10, opts.sigmaLB, opts.sigmaUB), ...
              inverse_bounded_transform(1.00, opts.lambdaLB, opts.lambdaUB)];
    thetaHat = fminsearch(obj, theta0, ...
        optimset('Display','off','MaxFunEvals',5000,'MaxIter',5000));

    sigma  = bounded_transform(thetaHat(1), opts.sigmaLB, opts.sigmaUB);
    lambda = bounded_transform(thetaHat(2), opts.lambdaLB, opts.lambdaUB);
    ...
    fit.loglik = loglik;
    fit.BIC    = BIC;
    fit.sigmaAtUpperBound = is_near_upper_bound(sigma, opts.sigmaLB, opts.sigmaUB, opts.boundTol);
end
...
function val = nll_type1(sigma, lambda, y, s, b0, opts)
    ...
    logdens = log_truncated_normal_pdf(y, mu, sigma, 0, 1, opts.epsFloor);
    val = -sum(logdens);
end
...
function logf = log_truncated_normal_pdf(x, mu, sigma, lower, upper, epsFloor)
    % Normal density conditional on the report lying in [lower, upper].
    % normcdf is written through erfc to avoid requiring the Statistics Toolbox.
    sigma = max(sigma, 1e-12);
    z = (x - mu) ./ sigma;
    cdfUpper = 0.5 .* erfc(-(upper - mu) ./ (sigma .* sqrt(2)));
    cdfLower = 0.5 .* erfc(-(lower - mu) ./ (sigma .* sqrt(2)));
    truncProb = max(cdfUpper - cdfLower, epsFloor);

    logf = -0.5 .* z.^2 - log(sqrt(2*pi) .* sigma) - log(truncProb);
    logf(x < lower | x > upper) = log(epsFloor);
    logf = max(logf, log(epsFloor));
end

function x = bounded_transform(theta, lower, upper)
    % Stable logistic map from the real line to (lower, upper).
    if theta >= 0
        p = 1 ./ (1 + exp(-theta));
    else
        e = exp(theta);
    ...
    x = lower + (upper - lower) .* p;
end

function theta = inverse_bounded_transform(x, lower, upper)
    p = (x - lower) ./ (upper - lower);
    p = min(max(p, 1e-10), 1 - 1e-10);
    theta = log(p ./ (1 - p));
end

function tf = is_near_upper_bound(x, lower, upper, relativeTol)
    tf = x >= upper - relativeTol .* (upper - lower);
end
...
%% Export bootstrap draws to Excel

bootstrap_table = table( ...
    shareRI(:,1),  shareRI(:,2),  shareRI(:,3), ...
    shareRIR(:,1), shareRIR(:,2), shareRIR(:,3), ...
    shareER(:,1),  shareER(:,2),  shareER(:,3), ...
    shareEI(:,1),  shareEI(:,2),  shareEI(:,3), ...
    meanSigma_RI(:,1),  meanSigma_RI(:,2),  meanSigma_RI(:,3), ...
    meanSigma_RIR(:,1), meanSigma_RIR(:,2), meanSigma_RIR(:,3), ...
    meanSigma_ER(:,1),  meanSigma_ER(:,2),  meanSigma_ER(:,3), ...
    meanSigma_EI(:,1),  meanSigma_EI(:,2),  meanSigma_EI(:,3), ...
    meanLambda_RI(:,1),  meanLambda_RI(:,2),  meanLambda_RI(:,3), ...
    meanLambda_RIR(:,1), meanLambda_RIR(:,2), meanLambda_RIR(:,3), ...
    'VariableNames', { ...
    'shareRI_C','shareRI_N','shareRI_F', ...
    'shareRIR_C','shareRIR_N','shareRIR_F', ...
    'shareER_C','shareER_N','shareER_F', ...
    'shareEI_C','shareEI_N','shareEI_F', ...
    'sigmaRI_C','sigmaRI_N','sigmaRI_F', ...
    'sigmaRIR_C','sigmaRIR_N','sigmaRIR_F', ...
    'sigmaER_C','sigmaER_N','sigmaER_F', ...
    'sigmaEI_C','sigmaEI_N','sigmaEI_F', ...
    'lambdaRI_C','lambdaRI_N','lambdaRI_F', ...
    'lambdaRIR_C','lambdaRIR_N','lambdaRIR_F'});

writetable(bootstrap_table, ...
    'bootstrap_results_matrix_truncated_normal.xlsx', ...
    'Sheet', 'bootstrap_draws');

fprintf('\nBootstrap draws exported to bootstrap_results_matrix_truncated_normal.xlsx\n');